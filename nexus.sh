...
# 检查并安装 Docker (兼容主流 Linux 发行版)
function check_docker() {
    if ! command -v docker >/dev/null 2>&1; then
        echo "检测到未安装 Docker，正在使用官方脚本安装..."
        # 使用官方的便利脚本，它能自动检测发行版并安装最新版的 Docker
        ...
        # 启动并设置开机自启 (适用于绝大多数使用 systemd 的现代 Linux 系统)
        echo "启动 Docker 服务并设置开机自启..."
        systemctl enable docker >/dev/null 2>&1 || echo "警告: 无法设置 Docker 开机自启，您可能需要手动操作。"
        systemctl start docker >/dev/null 2>&1 || echo "警告: 无法启动 Docker 服务，您可能需要手动操作。"
        
        if ! command -v docker >/dev/null 2>&1; then
            echo "错误：Docker 安装失败，请检查错误信息或尝试手动安装。"
            exit 1
        fi
        echo "Docker 安装成功！"
    fi
...
# 检查并安装 Node.js/npm/pm2 (兼容主流 Linux 发行版)
function check_pm2() {
    if ! command -v node >/dev/null 2>&1 || ! command -v npm >/dev/null 2>&1; then
        ...
        case $PKG_MANAGER in
            ...
            dnf)
                ...
                ;;
            yum)
                echo "提示: 在基于 YUM 的旧系统上 (如 CentOS 7), 可能需要配置 EPEL 源才能安装较新的 Node.js。"
                echo "正在尝试使用默认源安装..."
            ...
            zypper)
                ...
                ;;
            *)
                echo "错误：未检测到支持的包管理器。请手动安装 Node.js 和 npm。"
                exit 1
                ;;
        esac
        
        if ! command -v node >/dev/null 2>&1; then
            echo "错误：Node.js 安装失败，请尝试手动安装。"
            exit 1
        fi
    fi
