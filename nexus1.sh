...
# 构建docker镜像函数 (使用 Debian Slim 优化版)
...
# 优化 Debian 的软件源，切换到速度更快的全球镜像
RUN sed -i 's|http://deb.debian.org/debian/|http://mirrors.kernel.org/debian/|g' /etc/apt/sources.list.d/debian.sources

# 安装的软件包和命令与 Ubuntu 完全相同
...
function list_nodes() {
    echo "当前节点状态："
    printf "%-6s %-20s %-12s %-12s %-20s %s\n" "序号" "节点ID" "CPU" "内存" "状态" "启动时间"
    echo "-------------------------------------------------------------------------------------------------"
    ...
    for i in "${!all_nodes[@]}"; do
        ...
        if [ -n "$container_info" ]; then
            ...
            printf "%-6d %-20s %-12s %-12s %-20s %s\n" $((i+1)) "$node_id" "$cpu_usage" "$(echo "$mem_usage" | sed 's|/||g')" "$(echo "$status" | cut -d' ' -f1)" "$created_time"
        else
            if [ -n "$status" ]; then
                printf "%-6d %-20s %-12s %-12s %-20s %s\n" $((i+1)) "$node_id" "N/A" "N/A" "$(echo "$status" | cut -d' ' -f1)" "$created_time"
            fi
        fi
    done
    echo "-------------------------------------------------------------------------------------------------"
